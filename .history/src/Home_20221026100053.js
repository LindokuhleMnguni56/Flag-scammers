...
export default function HomeScreen({navigation}){
  ...
  const [liked,setLiked] = ReactuseState(false);
...
const eventInfo = ()=>{
  const getInfo = firebase.database().ref('/flags' +postKey)
  getInfo.on('value', snapshot => {
      console.log('like data: ', snapshot.val())
...
const like = () => {
  const updateLike = firebase.database().ref('/flags/' +postKey)
  updateLike.update({
      likes: likes +1 ,
      liked: true
  })};
               ...
               flags.map(flag=>((
                ...
                <View style={styles.cardsContainer} key={flag.id}>
                  <View style={styles.card}>
                        ...
                        <TouchableOpacity style={styles.upvoteBtn} onPress={like}>
