...
const AddScammer = () => {
    ...
    const addflag = async()=>{
        if (user!=null){
            //add to flag
            // docRef = await addDoc(itemRef,{address:mail ,userId:user.uid,addressType:selectedAddress, date:timestamp,comment:{[uid]:comment  }})
            // console.log("New Flag  ID: ", docRef.flag.uid);
            await addDoc(itemRef,{address:mail ,userId:user.uid,addressType:selectedAddress, date:timestamp})
            await addDoc(commentRef,{address:mail ,userId:user.uid,flagId:docRef.flag.id})
