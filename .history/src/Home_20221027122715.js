 ...
 const user=auth.currentUser
  ...
  const getItems = async () => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      let commentLikes = doc.data().comments.length
      //  let commentCount = 2
      console.log(commentCount);
      
      listFlag.push({ id: doc.id, address: doc.data().address, comment: doc.data().comments, date: doc.data().date, commentCount: commentCount,count})
    });
    setFlags(listFlag)
    setLoading(false)
